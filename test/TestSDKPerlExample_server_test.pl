...
use TestSDKPerlExample::TestSDKPerlExampleImpl;

local $| = 1;
my $token = $ENV{'KB_AUTH_TOKEN'};
my $config_file = $ENV{'KB_DEPLOYMENT_CONFIG'};
my $config = new Config::Simple($config_file)->get_block('TestSDKPerlExample');
...
$TestSDKPerlExample::TestSDKPerlExampleServer::CallContext = $ctx;
my $impl = new TestSDKPerlExample::TestSDKPerlExampleImpl();

sub get_ws_name {
    if (!defined($ws_name)) {
        my $suffix = int(time * 1000);
        $ws_name = 'test_TestSDKPerlExample_' . $suffix;
...

{
    ...
    sub provenance {
        my($self) = @_;
        return [{'service' => 'TestSDKPerlExample', 'method' => 'please_never_use_it_in_production', 'method_params' => []}];
    }
    sub authenticated {
        return 1;
    }
